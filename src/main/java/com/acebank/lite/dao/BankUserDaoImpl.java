...
public class BankUserDaoImpl implements BankUserDao {
    ...
    @Override
    public Optional<Integer> getAccountNoByEmail(String email) throws SQLException {
    ...
    @Override
    public boolean upsertPasswordResetOtp(String email, int accountNo, String otpHash, Timestamp expiresAt) throws SQLException {
        String sql = QueryLoader.get("user.password_reset_upsert");
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, email);
    ...
    @Override
    public Optional<PasswordResetOtp> getPasswordResetOtp(String email) throws SQLException {
        String sql = QueryLoader.get("user.password_reset_get");
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, email);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    return Optional.of(new PasswordResetOtp(
                            rs.getString("EMAIL"),
                            rs.getInt("ACCOUNT_NO"),
    ...
    @Override
    public boolean incrementPasswordResetAttempts(String email) throws SQLException {
        String sql = QueryLoader.get("user.password_reset_inc_attempt");
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, email);
            return ps.executeUpdate() > 0;
    ...
    @Override
    public boolean markPasswordResetUsed(String email) throws SQLException {
        String sql = QueryLoader.get("user.password_reset_mark_used");
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, email);
            return ps.executeUpdate() > 0;
    ...
    @Override
    public boolean setPasswordByAccountNo(int accountNo, String newPasswordHash) throws SQLException {
        String sql = QueryLoader.get("user.update_password");
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
